...
def lr_fb_selection(train, test, target_var, model_object, no_of_features, score_criteria) \
    ...
    sfs_forward = SequentialFeatureSelector(model_object,
                                            n_features_to_select=no_of_features,
                                            scoring=score_criteria,
                                            direction="forward").fit(x_train, y_train)

    selection_inst = (sfs_forward.get_support())
    ...
    filtered = pd.DataFrame(data=filtered, columns=[f"forward_selection"])
