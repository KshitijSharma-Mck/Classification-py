...
def dt_randomsearch(train, test, target_var, search_params_range)->(DecisionTreeClassifier,pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame,pd.DataFrame):
    ...
    tree = DecisionTreeClassifier(random_state=np.random.seed(42))
    ...
    DecisionTree_random_object = tree_cv.best_estimator_
    DecisionTree_random_object.fit(x_train, y_train)
    predicted_test = DecisionTree_random_object.fit(x_train, y_train).predict_proba(x_test)
    predicted_train = DecisionTree_random_object.fit(x_train, y_train).predict_proba(x_train)
    ...
    test_prediction = DecisionTree_random_object.predict(x_test)
    train_prediction = DecisionTree_random_object.predict(x_train)

    return DecisionTree_random_object, best_hyper_params, train_prediction, test_prediction, dt_train_predict, dt_test_predict
